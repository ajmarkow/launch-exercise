if (!customElements.get("localization-form")) {
  customElements.define(
    ...
    class LocalizationForm extends HTMLElement {
      constructor() {
        super();
        this.mql = globalThis.matchMedia("(min-width: 750px)");
      ...
      onContainerKeyDown(event) {
        const focusableItems = [...this.querySelectorAll("a")].filter(
          (item) => !item.parentElement.classList.contains("hidden"),
        );
        let focusedItemIndex = focusableItems.indexOf(document.activeElement);
        let itemToFocus;
      ...
      openSelector() {
        ...
        if (Object.hasOwn(this.dataset, "preventHide")) {
      ...
      filterCountries() {
        ...
        if (this.elements.liveRegion) {
          this.elements.liveRegion.innerHTML =
            globalThis.accessibilityStrings.countrySelectorSearchCount.replace(
              "[count]",
              visibleCountries,
            );
        }
