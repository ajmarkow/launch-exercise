...
source "/Users/ajmarkow/Documents/launch-engineer-exercise/.devenv/shell-env.sh"
...
export ZDOTDIR="/Users/ajmarkow/Documents/launch-engineer-exercise/.devenv/zsh"
...
if [ ! -x "/bin/zsh" ] && ! command -v "/bin/zsh" >/dev/null 2>&1; then
    echo "devenv: error: shell '/bin/zsh' not found" >&2
    echo "devenv: add zsh to your devenv.nix packages or set SHELL to an absolute path" >&2
    exit 1
fi
exec "/bin/zsh" -i
echo "devenv: error: failed to exec into /bin/zsh" >&2
exit 1
