...
const HTMLUpdateUtility = {
  /**
	 ...
	 @param oldNode
	 @param newContent
	 @param preProcessCallbacks
	 @param postProcessCallbacks
	 */
  viewTransition(
    oldNode,
    newContent,
    ...
    oldNode.querySelectorAll("[id], [form]").forEach((element) => {
      element.id &&= `${element.id}-${uniqueKey}`;
      element.form &&
  ...
  setInnerHTML(element, html) {
    ...
    element.querySelectorAll("script").forEach((oldScriptTag) => {
      ...
      for (const attribute of oldScriptTag.attributes) {
        newScriptTag.setAttribute(attribute.name, attribute.value);
      }

      newScriptTag.append(document.createTextNode(oldScriptTag.getHTML()));
...
Shopify.setSelectorByValue = function (selector, value) {
  for (let i = 0, count = selector.options.length; i < count; i++) {
    ...
    if (value == option.value || value == option.getHTML()) {
...
Shopify.addListener = function (target, eventName, callback) {
  target.addEventListener
    ? target.addEventListener(eventName, callback, { capture: false })
...
class BulkModal extends HTMLElement {
  ...
  connectedCallback() {
    const handleIntersection = (entries, observer) => {
      ...
      if (this.getHTML().trim() === "") {
        const productUrl = this.dataset.url.split("?", 1)[0];
        fetch(`${productUrl}?section_id=bulk-quick-order-list`)
          ...
          .then((responseText) => {
            ...
            this.innerHTML = sourceQty.getHTML();
          })
          .catch((error) => {
            console.error(error);
          });
...
class SliderComponent extends HTMLElement {
  ...
  initPages() {
    this.sliderItemsToShow = [...this.sliderItems].filter(
  ...
  togglePlayButtonState(pauseAutoplay) {
    if (pauseAutoplay) {
      ...
      this.sliderAutoplayButton.setAttribute(
        "aria-label",
        globalThis.accessibilityStrings.playSlideshow,
      );
    } else {
      this.sliderAutoplayButton.classList.remove("slideshow__autoplay--paused");
      this.sliderAutoplayButton.setAttribute(
        "aria-label",
        globalThis.accessibilityStrings.pauseSlideshow,
      );
    }
  }
...
class VariantSelects extends HTMLElement {
  ...
  dispatchProductSelectEvent() {
    const { ProductSelectEvent } = globalThis.StandardEvents || {};
    if (!ProductSelectEvent) {
      return;
    }
  ...
  updateSelectionMetadata({ target }) {
    ...
    if (tagName === "SELECT" && target.selectedOptions.length > 0) {
      [...target.options]
  ...
  loadRecommendations(productId) {
    fetch(
      ...
      .then((text) => {
        ...
        if (recommendations?.getHTML().trim().length) {
          this.innerHTML = recommendations.getHTML();
        }
...
class BulkAdd extends HTMLElement {
  ...
  startCartLinesUpdate(items) {
    const { CartLinesUpdateEvent } = globalThis.StandardEvents || {};
      ...
      (groups, [variantId, quantity]) => {
        ...
        if (action === "add") {
          ...
          const lineKey = this.querySelector(
            `[data-quantity-variant-id="${CSS.escape(variantId)}"]`,
  ...
  dispatchCartErrorEvent(message, code) {
    const { CartErrorEvent } = globalThis.StandardEvents || {};
    if (!CartErrorEvent) {
      return;
    }
  ...
  validateQuantity(event) {
    ...
    if (inputValue < event.target.dataset.min) {
      this.setValidity(
        event,
        index,
        globalThis.quickOrderListStrings.min_error.replace(
          "[min]",
      ...
      this.setValidity(
        event,
        index,
        globalThis.quickOrderListStrings.max_error.replace(
          "[max]",
          event.target.max,
        ),
      );
      ...
      this.setValidity(
        event,
        index,
        globalThis.quickOrderListStrings.step_error.replace(
          "[step]",
          event.target.step,
        ),
      );
