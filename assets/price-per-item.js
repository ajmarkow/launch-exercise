        ...
        for (const pair of this.qtyPricePairs) {
          if (this.currentQtyForVolumePricing >= pair[0]) {
            const pricePerItemCurrent = document.querySelector(
              `price-per-item[id^="Price-Per-Item-${CSS.escape(this.dataset.sectionId || this.dataset.variantId)}"] .price-per-item span`,
            );
            this.classList.contains("variant-item__price-per-item")
              ? (pricePerItemCurrent.innerHTML =
                  globalThis.quickOrderListStrings.each.replace(
                    "[money]",
                    pair[1],
                  ))
