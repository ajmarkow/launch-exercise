...
function collectCartSections() {
  ...
  for (const element of document.querySelectorAll(DAWN_CART_TAGS.join(","))) {
    ...
    for (const entry of entries ?? []) {
      if (DAWN_PUBSUB_REFRESHED_SECTIONS.has(`${tag}:${entry.id}`)) {
        continue;
      }
...
async function refreshDawnCartUI() {
  ...
  const sectionsQuery =
    sections.size > 0 ? `?sections=${[...sections.keys()].join(",")}` : "";
  // `routes` is a Dawn global, but don't assume it's defined.
  const cartUrl =
    (typeof routes !== "undefined" && routes?.cart_url) || "/cart";
...
function initStandardActions() {
  const actions = globalThis.Shopify?.actions;
  if (!actions) {
    return;
  }
  ...
  actions.updateCart.configure({
    ...
    async handler(defaultHandler) {
      ...
      try {
        ...
        console.error(
          "[Dawn] Standard Actions cart refresh failed; reloading.",
          error,
        );
...
if (globalThis.Shopify?.actions) {
  initStandardActions();
} else {
  document.addEventListener("DOMContentLoaded", initStandardActions, {
    once: true,
  });
}
