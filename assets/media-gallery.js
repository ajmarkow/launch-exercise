    ...
    class MediaGallery extends HTMLElement {
      ...
      onSlideChanged(event) {
        const thumbnail = this.elements.thumbnails.querySelector(
          `[data-target="${CSS.escape(event.detail.currentElement.dataset.mediaId)}"]`,
        );
        ...
        if (prepend) {
          ...
          if (this.elements.thumbnails) {
            const activeThumbnail = this.elements.thumbnails.querySelector(
              `[data-target="${CSS.escape(mediaId)}"]`,
            );
      ...
      announceLiveRegion(activeItem, position) {
        ...
        image.addEventListener("load", () => {
          ...
          this.elements.liveRegion.innerHTML =
            globalThis.accessibilityStrings.imageAvailable.replace(
              "[index]",
              position,
            );
          setTimeout(() => {
