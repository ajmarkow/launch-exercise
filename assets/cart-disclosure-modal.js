  ...
  class CartDisclosureModal extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.opener = this.previousElementSibling?.classList.contains(
        ...
        : controlsId
          ? document.querySelector(
              `[aria-controls="${CSS.escape(controlsId)}"]`,
            )
          : null;
      ...
      document.querySelectorAll("cart-disclosure-modal").forEach((modal) => {
        if (modal === this || modal.id !== this.id) {
          return;
        }
    ...
    removeIfOpenerDisconnected() {
      if (this.isMoving || !this.isConnected || this.opener?.isConnected) {
        return;
      }
    ...
    restoreFocusTrap() {
      ...
      const cartNotification = this.opener
