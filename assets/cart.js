...
class CartItems extends globalThis.StandardEvents.createViewEventElement(
  HTMLElement,
) {
  constructor() {
    super();
    this.lineItemStatusElement =
      document.querySelector("#shopping-cart-line-item-status") ||
      document.querySelector("#CartDrawer-LineItemStatus");
  ...
  validateQuantity(event) {
    ...
    if (inputValue < event.target.dataset.min) {
      message = globalThis.quickOrderListStrings.min_error.replace(
        "[min]",
        event.target.dataset.min,
      );
    } else if (inputValue > parseInt(event.target.max)) {
      message = globalThis.quickOrderListStrings.max_error.replace(
        "[max]",
        event.target.max,
      );
    } else if (inputValue % parseInt(event.target.step) !== 0) {
      message = globalThis.quickOrderListStrings.step_error.replace(
        "[step]",
        event.target.step,
      );
    }
  ...
  getSectionsToRender() {
    return [
      {
        id: "main-cart-items",
        section: document.querySelector("#main-cart-items").dataset.id,
      ...
      {
        id: "main-cart-footer",
        section: document.querySelector("#main-cart-footer").dataset.id,
        selector: ".js-contents",
      },
    ];
  }
  ...
  updateQuantity(line, quantity, event, name, variantId) {
    ...
    fetch(`${routes.cart_change_url}`, { ...fetchConfig(), body })
        ...
        CartPerformance.measure(`${eventTarget}:paint-updated-sections`, () => {
          ...
          if (
            ...
            if (updatedValue === undefined) {
              ...
              message = globalThis.cartStrings.quantityError.replace(
                "[quantity]",
                updatedValue,
              );
            }
          ...
          if (
            lineItem &&
            lineItem.querySelector(`[name="${CSS.escape(name)}"]`)
          ) {
            cartDrawerWrapper
              ? trapFocus(
                  cartDrawerWrapper,
              ...
              : lineItem.querySelector(`[name="${CSS.escape(name)}"]`).focus();
          } else if (
            parsedState.item_count === 0 &&
      ...
      .catch((error) => {
        ...
        const errors =
          document.querySelector("#cart-errors") ||
          document.querySelector("#CartDrawer-CartErrors");
        if (errors) {
          errors.textContent = globalThis.cartStrings.error;
        }

        this.dispatchCartErrorEvent(
          globalThis.cartStrings.error,
          "SERVICE_UNAVAILABLE",
        );
  ...
  createCartLinesUpdateEvent(action, variantId, quantity, lineKey) {
    ...
    if (!CartLinesUpdateEvent || !variantId) {
      return null;
    }
  ...
  updateLiveRegions(line, message) {
    ...
    const cartStatus =
      ...
      document.querySelector("#CartDrawer-LiveRegionText");
  ...
  enableLoading(line) {
    ...
    for (const overlay of [...cartItemElements, ...cartDrawerItemElements]) {
  ...
  customElements.define(
    ...
    class CartNote extends HTMLElement {
      ...
      dispatchNoteUpdateEvent(newNote) {
        const { CartNoteUpdateEvent } = globalThis.StandardEvents || {};
        if (!CartNoteUpdateEvent) {
          return null;
        }
