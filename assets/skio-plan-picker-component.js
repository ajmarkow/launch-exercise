...
export class SkioPlanPicker extends LitElement {
  ...
  constructor() {
    ...
    this.debug = this.debug || globalThis?.Shopify?.designMode;
      ...
      >
                ...
                <label class="group-label" for="one-time-${this.key}">
                  <div class="group-topline">
                    ...
                    <div class="skio-price" aria-live="polite">
                      ...
                      ${this.options?.show_unit_price
                        ? this.unitPriceTemplate()
                        : ""}
                    </div>
                  </div>
                </label>
        ...
        ${this.availableSellingPlanGroups?.length
          ? this.availableSellingPlanGroups.map(
              (group, index) => html`
                <div
                  class="group-container ${this.selectedSellingPlanGroup ==
                  group
                    ? "group-container--selected"
                    : ""}"
                >
                  ...
                  <label class="group-label" for="group-${index}-${this.key}">
                    <div class="group-topline">
                      ...
                      <div class="group-title">
                        ...
                        ${this.tooltipContent
                          ? html`<span
                              class="skio-tooltip"
                              tabindex="0"
                              aria-describedby="skio-tooltip-${this
                                .key}-${index}"
                            >
                              <svg
                                width="16"
                              ...
                              >
                                <path
                                  d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zM7 4h2v2H7V4zm0 3h2v5H7V7z"
                                />
                              </svg>
                              <span
                                id="skio-tooltip-${this.key}-${index}"
                                class="skio-tooltip-text"
                                role="tooltip"
                                >${this.tooltipContent}</span
                              >
                            </span>`
                          : ""}
                      </div>
                      ...
                      <div class="skio-price" aria-live="polite">
                        ...
                        ${this.options?.show_unit_price
                          ? this.unitPriceTemplate(group.selected_selling_plan)
                          : ""}
                      </div>
                    </div>
  ...
  async fetchRules() {
    ...
    const rules = await fetch(
      `https://api.skio.com/storefront-http/get-rules-by-domain-or-hostname?domain=${globalThis?.Shopify?.shop}`,
    )
    ...
    this.rules.forEach((rule, index) => {
      if (
        !(
          ...
          rule.minQuantityToDiscount <= this.eligibleItemCount(rule)
        )
      ) {
        return;
      }
  ...
  bindFormEvents() {
    ...
    if (!this.variantInput) {
      this.variantInput = document.querySelector(
        `[name="id"][form="${CSS.escape(this.options?.form_id)}"]`,
      );
    }
    ...
    document.addEventListener("variantChanged", function (e) {
      ...
      const variant = skio.product.variants.find((x) => x.id == variantId);
      if (variant) {
        skio.selectedVariant = variant;
      } else {
        skio.log("Unable to find variant with id: ", variantId);
      }
  ...
  getAllProductForms() {
    return [...document.forms].filter((form) => {
  ...
  detailsMouseover() {
    const details = this.renderRoot.querySelector(".skio-details");
    const summary = this.renderRoot.querySelector(".skio-details summary");
    ...
    if (changed.has("selectedVariant") && this.selectedVariant) {
      ...
      if (this.availableSellingPlanGroups?.length) {
        ...
        this.availableSellingPlanGroups.forEach((group) => {
          ...
          group.selected = Boolean(isAnyPlanSelected);
        });
  ...
  updateExternalElements() {
    document
      .querySelectorAll(`[skio-price][skio-key="${CSS.escape(this.key)}"]`)
      .forEach((element) => {
        element.innerHTML = this.price(this.selectedSellingPlan);
      });

    document
      .querySelectorAll("[skio-subscription-content]")
      .forEach((element) => {
        element.style.display = this.selectedSellingPlan ? "block" : "none";
      });
  }
  ...
  updateExternalPrice() {
    document
      ...
      .forEach((element) => {
        this.selectedSellingPlan
          ? (element.innerHTML = this.price(this.selectedSellingPlan))
          : (element.innerHTML = this.money(this.selectedVariant.price));
      });
  }
  ...
  discountText(selling_plan) {
    if (!this.options?.discount_text || this.options?.discount_text == "") {
      return;
    }
  ...
  postCheckoutDiscountsText(sellingPlan = this.selectedSellingPlan) {
    ...
    if (postCheckoutPriceAdjustments.length === 0) {
      return;
    }
  ...
  getDiscountFromPriceAdjustment(selling_plan, price_adjustment) {
    ...
    const sellingPlanGroup = this.product.selling_plan_groups.find((group) =>
      group.selling_plans.some((plan) => plan.id == selling_plan.id),
    );

    if (sellingPlanGroup?.name === "Prepaid") {
      const string_ = selling_plan.name; // Replace with your string
      const intervalDate = /\b\d+\s*(days|weeks|months|years)\b/gi;
      const intervalDateMatches = string_.match(intervalDate);
    ...
    switch (price_adjustment.value_type) {
      case "percentage": {
        ...
        discount.absolute = Math.round((price * price_adjustment.value) / 100);
        discount.amount = Math.round((price * price_adjustment.value) / 100);
        break;
      }

      case "fixed_amount": {
        discount.percent = Math.round(
          ((price_adjustment.value * 1) / price) * 100,
        );
      ...
      case "price": {
        discount.percent = Math.round(
          (((compareAtPrice * multiplier - price_adjustment.value) * 1) /
  ...
  money(cents, formatString = this.moneyFormat) {
    ...
    function formatWithDelimiters(
      ...
      const dollars = parts[0].replaceAll(
        /(\d)(?=(\d\d\d)+(?!\d))/g,
        "$1" + thousands,
      );
  ...
  unitPriceTemplate(selling_plan = null) {
    ...
    if (selling_plan) {
      ...
      displayPrice = Math.round(
        (unitPrice * subscriptionPrice) / this.selectedVariant.price,
      );
    }

    return html`<div class="skio-unit-price">
      ${this.money(displayPrice)}/${measurement.reference_unit}
    </div>`;
  }

  // Updates element data to be registered by forms
  updateForm() {
    const $sellingPlan = this.querySelector("input[name=selling_plan]");
  ...
  updateTooltipContent() {
    ...
    if (!plan && this.availableSellingPlanGroups?.length) {
      ...
      plan =
        firstGroup.selected_selling_plan ||
        firstGroup.selling_plans?.[0] ||
        null;
    }
...
if (globalThis) {
  globalThis.SkioPlanPicker = SkioPlanPicker;
}
