    ...
    class ProductInfo extends HTMLElement {
      ...
      initializeProductSwapUtility() {
        ...
        this.postProcessHtmlCallbacks.push((newNode) => {
          globalThis?.Shopify?.PaymentButton?.init();
      ...
      handleSwapProduct(productUrl, updateFullPage) {
        return (html) => {
          ...
          if (updateFullPage) {
            document.querySelector("head title").innerHTML = html
      ...
      buildRequestUrlWithParams(
        ...
        !shouldFetchFullPage && parameters.push(`section_id=${this.sectionId}`);

        if (optionValues.length > 0) {
          parameters.push(`option_values=${optionValues.join(",")}`);
        }
        ...
        return (html) => {
          ...
          const updateSourceFromDestination = (
            ...
            if (source && destination) {
              destination.innerHTML = source.getHTML();
      ...
      updateVariantInputs(variantId) {
        this.querySelectorAll(
          `#shop-form-${this.dataset.section}, #product-form-${this.dataset.section}`,
        ).forEach((productForm) => {
      ...
      updateURL(url, variantId) {
        this.querySelector("share-button")?.updateUrl(
          `${globalThis.shopUrl}${url}${variantId ? `?variant=${variantId}` : ""}`,
        );
      ...
      setUnavailable() {
        this.productForm?.toggleSubmitButton(
          true,
          globalThis.variantStrings.unavailable,
        );
      ...
      updateMedia(html, variantFeaturedMediaId) {
        ...
        const refreshSourceData = () => {
          if (Object.hasOwn(this.dataset, "zoomOnHover")) {
            enableZoomOnHover(2);
          }
        ...
        if (mediaGallerySource && mediaGalleryDestination) {
          ...
          for (const mediaGallerySourceItem of mediaGallerySourceItems) {
            if (destinationSet.has(mediaGallerySourceItem.dataset.mediaId)) {
              continue;
            }

            mediaGallerySourceItem.remove();
            shouldRefresh = true;
          }
          ...
          for (const [
            destinationIndex,
            destinationItem,
          ] of mediaGalleryDestinationItems.entries()) {
        ...
        if (modalContent && newModalContent) {
          modalContent.innerHTML = newModalContent.getHTML();
        }
      }
      ...
      updateQuantityRules(sectionId, html) {
        ...
        for (const selector of selectors) {
          ...
          if (selector === ".quantity__input") {
            ...
            current.innerHTML = updated.getHTML();
      ...
      get relatedProducts() {
        ...
        return document.querySelector(
          `product-recommendations[data-section-id^="${CSS.escape(relatedProductsSectionId)}"]`,
        );
      }
      ...
      get quickOrderList() {
        ...
        return document.querySelector(
          `quick-order-list[data-id^="${CSS.escape(quickOrderListSectionId)}"]`,
        );
      }
